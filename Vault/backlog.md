...
- [ ] Restore fade-out animation on filter dropdowns — currently conditionally rendered (instant close) to prevent ghost hover over map markers. Need a delayed unmount pattern so panels can fade out before being removed from the DOM. #p3
...
- [ ] Live energy monitoring feeds on project pages — embed inverter dashboards (e.g. SMA Sunny Portal) to show real-time solar production. Ask NGO partners what monitoring platforms they use. RePower Ukraine uses Sunny Portal; Ecoclub/Ecoaction/Greenpeace likely have inverter monitoring but don't expose it publicly. #p2 @tom
