...
**Sprint:** 2026-03-18 → 2026-03-31
**Focus:** Post-launch momentum, FSA signing, payment processing prep
...
- [/] POCACITO board review and signing — board meets Mar 27 #p0 @tom
- [ ] Obtain real bank details from POCACITO post-signing #p0 @tom
- [ ] Replace placeholder bank details in SupportProjectCard #p0 @tom
- [ ] Restore POCACITO logo, Candid seal, and legal text to email footer post-FSA #p1 @tom
- [ ] Restore POCACITO/FSA references on site post-FSA #p1 @tom
...
- [ ] Add Proton DKIM CNAME records (3 records, sent to Sloan) — prevents spam filtering #p1 @tom

### Technical
- [ ] Fix GitHub Actions CI failures (security workflow, lint, type check) #p2 @tom
...
### Launch Day (Mar 18) — Post-Launch Fixes
- [x] Add RENOVATION and WATER_TREATMENT to projectTypes in en.json and uk.json #p1 @tom
- [x] Add RENOVATION and WATER_TREATMENT to ProjectType union type and PROJECT_TYPE_CONFIG #p1 @tom
- [x] Upload and attach photos to Poromiv Lyceum, Zabolotsi Lyceum, Shakhtar Sports Complex #p1 @tom
- [x] Create white logo variants for Greenpeace and Ecoclub (transparent PNG) #p2 @tom
...
- [x] Draft and publish launch post — Hromada + mission + Ecoaction + projects #p0 @tom
- [x] Remove site password for open access (removed SITE_PASSWORD env var) #p0 @tom
- [x] Merge directory-mode → main (force-push, 246 commits) #p0 @tom
- [x] Set up Resend email provider — domain verified, DKIM/SPF/MX configured #p0 @tom
- [x] Set up hromadaproject.org email — Proton custom domain, thomas@ and info@ addresses #p1 @tom
- [x] Hide POCACITO branding from email footer (pre-FSA) #p1 @tom
- [x] Fix Amplify env vars — RESEND_API_KEY, EMAIL_PROVIDER, SESSION_SECRET added to runtime #p1 @tom
- [x] Newsletter signup working end-to-end via Resend #p1 @tom
- [x] Admin login working #p1 @tom

### Previous Sprint (Mar 3–17)
...
### Phase 1: Soft Launch — COMPLETE (March 18, 2026)
Matchmaking directory launched. Ecoaction as first official partner. No payment processing, no donation flow, no FSA/POCACITO references. Launched across socials.

**LinkedIn rollout (sequential posts, showing momentum):**
1. **Launch post** — DONE. Hromada + mission + Ecoaction + first projects.
...
- Restore POCACITO branding on site and emails.

### Phase 3: Payment Processing Live
- Add real bank details from POCACITO.
- Post: donation flow is live, projects can now be funded.

### Phase 4: Ongoing Cadence
- May 20 Cannon Building event announcement
- Newsletter first edition
- Project funded / project built announcements
...
| NGO partner MoUs | @tom | Verbally reviewed, at partner boards. Not urgent pre-FSA. |
| May 20 event prep | @tom @max | Cannon Building booked. Need speaker confirmations, invites, materials. |
| Proton DKIM records | @sloan | 3 CNAME records sent, not yet added to Route 53 |
